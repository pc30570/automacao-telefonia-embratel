            ...
            for tr in trio_results:
                ...
                with st.expander(f"{icon} {tr.key}  —  {len(tr.rows)} linha(s)"):
                    if tr.total_boleto is not None:
                        st.write(f"**Total do boleto:** R$ {tr.total_boleto:,.2f}")
                    if tr.total_agregado is not None:
                        st.write(f"**Total agregado (RPS + NF):** R$ {tr.total_agregado:,.2f}")
                    if tr.diferenca is not None:
                        st.write(f"**Diferença:** R$ {tr.diferenca:,.2f}")
